...
brute_force(You, Boss, Store) ->
    Combs = [{W, A, R1, R2} || W <- lists:seq(1, length(Store#store.weapons)),
                               ...
                               R1 =/= R2],
    lists:foldl(fun({W, A, R1, R2}, {{AccW, AccA, AccR1, AccR2}, Min}) ->
        ...
        case fight(You#pers{hit_points=You#pers.hit_points,
                   ...
                   Boss) of
            win -> case cost(Weapon, Armor, Ring1, Ring2) of
                       X when X < Min -> {{W, A, R1, R2}, X};
                       _ -> {{AccW, AccA, AccR1, AccR2}, Min}
                   end;
            lost -> {{AccW, AccA, AccR1, AccR2}, Min}
        end
    end, {{-1, -1, -1, -1}, min}, Combs).

calc(You, Boss, Store) ->
    {{_Weapon, _Armor, _Ring1, _Ring2}, Min} = brute_force(You, Boss, Store),
%    {{Weapon, Armor, Ring1, Ring2}, Min} = brute_force(You, Boss, Store),
...
%    {{W#item.name, A#item.name, R1#item.name, R2#item.name}, Min}.
    Min.
